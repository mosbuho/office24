...
create sequence refund_seq start with 1 increment by 1 nocache nocycle;
...
create table refund (
	...
	member_no     number
		references member ( no )
			on delete cascade,                                          -- 멤버 번호
	name          varchar2(12) not null,                                -- 예약자
	phone         char(11) not null,                                    -- 예약자 번호
	price         number(10) not null,                                  -- 예약 금액
	refund_amount number(10) not null,                                  -- 환불 금액
	payment       varchar2(20) not null,                                -- 결제 수단
	start_date    date not null,                                        -- 예약 시작일
	end_date      date not null,                                        -- 예약 종료일
	booking_date  date,                                                 -- 예약일
	reg_date      date default sysdate                                  -- 환불 처리 일자
);
...
create table sales_statistics (
	report_date  date primary key,
	sales_create number default 0,
	sales_delete number default 0
);
